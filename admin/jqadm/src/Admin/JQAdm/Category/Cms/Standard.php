...
namespace Aimeos\Admin\JQAdm\Category\Cms;

sprintf( 'cms' ); // for translation


/**
 * Default implementation of category cms JQAdm client.
 *
 * @package Admin
 * @subpackage JQAdm
 */
...
{
	/** admin/jqadm/category/cms/name
	 * Name of the cms subpart used by the JQAdm category implementation
	 *
	 * Use "Myname" if your class is named "\Aimeos\Admin\Jqadm\Category\Cms\Myname".
	...
	public function addData( \Aimeos\MW\View\Iface $view ) : \Aimeos\MW\View\Iface
	{
		...
		$search->setConditions( $search->compare( '==', 'category.lists.type.domain', 'cms' ) );
		$search->setSortations( [$search->sort( '+', 'category.lists.type.position' )] );

		$view->cmsListTypes = $manager->search( $search );
	...
	public function get() : ?string
	{
		...
		$total = 0;
		$params = $this->storeFilter( $view->param( 'cp', [] ), 'categorycms' );
		$listItems = $this->getListItems( $view->item, $params, $total );

		$view->cmsItems = $this->getCmsItems( $listItems );
		$view->cmsData = $this->toArray( $listItems );
		$view->cmsBody = parent::get();
		$view->cmsTotal = $total;
	...
	public function save() : ?string
	{
		...
		try
		{
			$this->storeFilter( $view->param( 'cp', [] ), 'categorycms' );
			$this->fromArray( $view->item, $view->param( 'cms', [] ) );
			$view->cmsBody = parent::save();
	...
	{
		/** admin/jqadm/category/cms/decorators/excludes
		 ...
		 *
		 *  admin/jqadm/category/cms/decorators/excludes = array( 'decorator1' )
		 *
		 ...
		 * @see admin/jqadm/category/cms/decorators/global
		 * @see admin/jqadm/category/cms/decorators/local
		 */

		/** admin/jqadm/category/cms/decorators/global
		 ...
		 *
		 *  admin/jqadm/category/cms/decorators/global = array( 'decorator1' )
		 *
		 ...
		 * @category Developer
		 * @see admin/jqadm/common/decorators/default
		 * @see admin/jqadm/category/cms/decorators/excludes
		 * @see admin/jqadm/category/cms/decorators/local
		 */

		/** admin/jqadm/category/cms/decorators/local
		 ...
		 *
		 *  admin/jqadm/category/cms/decorators/local = array( 'decorator2' )
		 *
		 ...
		 * @see admin/jqadm/category/cms/decorators/global
		 */
		return $this->createSubClient( 'category/cms/' . $type, $name );
	}


	/**
	 * Returns the category list items referencing the cmss
	 *
	 ...
	 * @return \Aimeos\Map Category list items implementing \Aimeos\MShop\Common\Item\List\Iface  referencing the cmss
	 */
	protected function getListItems( \Aimeos\MShop\Category\Item\Iface $item, array $params = [], &$total = null ) : \Aimeos\Map
	{
		...
		$search = $this->initCriteria( $search, $params, 'categorycms' );
		$expr = [
	...
	/**
	 * Returns the cms items referenced by the given list items
	 *
	 * @param \Aimeos\Map $listItems Category list items implementing \Aimeos\MShop\Common\Item\List\Iface and referencing the cmss
	 * @return \Aimeos\Map List of cms IDs as keys and items implementing \Aimeos\MShop\Cms\Item\Iface
	 */
	protected function getCmsItems( \Aimeos\Map $listItems ) : \Aimeos\Map
	{
		$list = $listItems->getRefId()->toArray();
		$manager = \Aimeos\MShop::create( $this->getContext(), 'cms' );

		$search = $manager->filter()->slice( 0, count( $list ) );
		$search->setConditions( $search->compare( '==', 'cms.id', $list ) );
	...
	{
		/** admin/jqadm/category/cms/subparts
		 * List of JQAdm sub-clients rendered within the category cms section
		 *
		 ...
		 * the JQAdm code of its sub-clients. The cms of the JQAdm sub-clients
		 * determines the cms of the output of these sub-clients inside the parent
		 * container. If the configured list of clients is
		 *
		 *  array( "subclient1", "subclient2" )
		 *
		 * you can easily change the cms of the output by recmsing the subparts:
		 *
		 ...
		 * should support adding, removing or recmsing content by a fluid like
		 * design.
		 *
		 ...
		 * @category Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/category/cms/subparts', [] );
	}
	 ...
	 *
	 * @param \Aimeos\Map $listItems Category list items implementing \Aimeos\MShop\Common\Item\Lists\Iface and referencing the cmss
	...
	{
		/** admin/jqadm/category/cms/template-item
		 * Relative path to the HTML body template of the cms subpart for categorys.
		 *
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/category/cms/template-item';
		$default = 'category/item-cms-standard';
