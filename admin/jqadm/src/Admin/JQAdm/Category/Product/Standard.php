...
namespace Aimeos\Admin\JQAdm\Category\Product;

sprintf( 'product' ); // for translation


/**
 * Default implementation of category product JQAdm client.
 *
 * @package Admin
 * @subpackage JQAdm
 */
...
{
	/** admin/jqadm/category/product/name
	 * Name of the product subpart used by the JQAdm category implementation
	 *
	 * Use "Myname" if your class is named "\Aimeos\Admin\Jqadm\Category\Product\Myname".
	...
	public function addData( \Aimeos\MW\View\Iface $view ) : \Aimeos\MW\View\Iface
	{
		...
		$search->setConditions( $search->compare( '==', 'category.lists.type.domain', 'product' ) );
	...
	public function get() : ?string
	{
		...
		$total = 0;
		$params = $this->storeFilter( $view->param( 'cp', [] ), 'categoryproduct' );
	...
	public function save() : ?string
	{
		...
		try
		{
			$this->storeFilter( $view->param( 'cp', [] ), 'categoryproduct' );
	...
	{
		/** admin/jqadm/category/product/decorators/excludes
		 ...
		 *
		 *  admin/jqadm/category/product/decorators/excludes = array( 'decorator1' )
		 *
		 ...
		 * @see admin/jqadm/category/product/decorators/global
		 * @see admin/jqadm/category/product/decorators/local
		 */

		/** admin/jqadm/category/product/decorators/global
		 ...
		 *
		 *  admin/jqadm/category/product/decorators/global = array( 'decorator1' )
		 *
		 ...
		 * @category Developer
		 * @see admin/jqadm/common/decorators/default
		 * @see admin/jqadm/category/product/decorators/excludes
		 * @see admin/jqadm/category/product/decorators/local
		 */

		/** admin/jqadm/category/product/decorators/local
		 ...
		 *
		 *  admin/jqadm/category/product/decorators/local = array( 'decorator2' )
		 *
		 ...
		 * @see admin/jqadm/category/product/decorators/global
		 */
		return $this->createSubClient( 'category/product/' . $type, $name );
	}


	/**
	 * Returns the category list items referencing the products
	 *
	 ...
	 * @return \Aimeos\Map Category list items implementing \Aimeos\MShop\Common\Item\List\Iface  referencing the products
	 */
	protected function getListItems( \Aimeos\MShop\Category\Item\Iface $item, array $params = [], &$total = null ) : \Aimeos\Map
	{
		...
		$search = $this->initCriteria( $search, $params, 'categoryproduct' );
		$expr = [
			$search->getConditions(),
			$search->compare( '==', 'category.lists.parentid', $item->getId() ),
			$search->compare( '==', 'category.lists.domain', 'product' ),
		];
	 ...
	 *
	 * @param \Aimeos\Map $listItems Category list items implementing \Aimeos\MShop\Common\Item\List\Iface and referencing the products
	...
	{
		/** admin/jqadm/category/product/subparts
		 * List of JQAdm sub-clients rendered within the category product section
		 *
		 ...
		 * @category Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/category/product/subparts', [] );
	}
	 ...
	 *
	 * @param \Aimeos\Map $listItems Category list items implementing \Aimeos\MShop\Common\Item\Lists\Iface and referencing the products
	...
	{
		/** admin/jqadm/category/product/template-item
		 * Relative path to the HTML body template of the product subpart for categorys.
		 *
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/category/product/template-item';
		$default = 'category/item-product-standard';
