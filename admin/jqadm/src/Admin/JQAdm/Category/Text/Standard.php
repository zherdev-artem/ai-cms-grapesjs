...
namespace Aimeos\Admin\JQAdm\Category\Text;

sprintf( 'text' ); // for translation


/**
 * Default implementation of category text JQAdm client.
 *
 * @package Admin
 * @subpackage JQAdm
 */
...
{
	/** admin/jqadm/category/text/name
	 * Name of the text subpart used by the JQAdm category implementation
	 *
	 * Use "Myname" if your class is named "\Aimeos\Admin\Jqadm\Category\Text\Myname".
	...
	{
		/** admin/jqadm/category/text/decorators/excludes
		 ...
		 *
		 *  admin/jqadm/category/text/decorators/excludes = array( 'decorator1' )
		 *
		 ...
		 * @see admin/jqadm/category/text/decorators/global
		 * @see admin/jqadm/category/text/decorators/local
		 */

		/** admin/jqadm/category/text/decorators/global
		 ...
		 *
		 *  admin/jqadm/category/text/decorators/global = array( 'decorator1' )
		 *
		 ...
		 * @category Developer
		 * @see admin/jqadm/common/decorators/default
		 * @see admin/jqadm/category/text/decorators/excludes
		 * @see admin/jqadm/category/text/decorators/local
		 */

		/** admin/jqadm/category/text/decorators/local
		 ...
		 *
		 *  admin/jqadm/category/text/decorators/local = array( 'decorator2' )
		 *
		 ...
		 * @see admin/jqadm/category/text/decorators/global
		 */
		return $this->createSubClient( 'category/text/' . $type, $name );
	}
	...
	{
		/** admin/jqadm/category/text/subparts
		 * List of JQAdm sub-clients rendered within the category text section
		 *
		 ...
		 * @category Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/category/text/subparts', [] );
	}
	...
	public function addData( \Aimeos\MW\View\Iface $view ) : \Aimeos\MW\View\Iface
	{
		...
		$search->setConditions( $search->compare( '==', 'text.type.domain', 'category' ) );
		$search->setSortations( [$search->sort( '+', 'text.type.position' )] );

		$listSearch = $listTypeManager->filter( true )->slice( 0, 10000 );
		$listSearch->setConditions( $listSearch->compare( '==', 'category.lists.type.domain', 'text' ) );
	...
	protected function fromArray( \Aimeos\MShop\Category\Item\Iface $item, array $data ) : \Aimeos\MShop\Category\Item\Iface
	{
		...
		foreach( $data as $idx => $entry )
		{
			...
			if( ( $listItem = $item->getListItem( 'text', $entry['category.lists.type'], $entry['text.id'], false ) ) === null ) {
	...
	{
		/** admin/jqadm/category/text/template-item
		 * Relative path to the HTML body template of the text subpart for categorys.
		 *
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/category/text/template-item';
		$default = 'category/item-text-standard';
