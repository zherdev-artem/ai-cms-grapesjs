...
namespace Aimeos\Admin\JQAdm\Post\Media;

sprintf( 'media' ); // for translation


/**
 * Default implementation of post media JQAdm client.
 *
 * @package Admin
 * @subpackage JQAdm
 */
...
{
	/** admin/jqadm/post/media/name
	 * Name of the media subpart used by the JQAdm post implementation
	 *
	 * Use "Myname" if your class is named "\Aimeos\Admin\Jqadm\Post\Media\Myname".
	...
	/**
	 * Adds the required data used in the post template
	 *
	...
	public function addData( \Aimeos\MW\View\Iface $view ) : \Aimeos\MW\View\Iface
	{
		...
		$search->setConditions( $search->compare( '==', 'media.type.domain', 'post' ) );
		$search->setSortations( [$search->sort( '+', 'media.type.position' )] );

		$listSearch = $listTypeManager->filter( true )->slice( 0, 10000 );
		$listSearch->setConditions( $listSearch->compare( '==', 'post.lists.type.domain', 'media' ) );
	...
	{
		/** admin/jqadm/post/media/decorators/excludes
		 ...
		 *
		 *  admin/jqadm/post/media/decorators/excludes = array( 'decorator1' )
		 *
		 ...
		 * @see admin/jqadm/post/media/decorators/global
		 * @see admin/jqadm/post/media/decorators/local
		 */

		/** admin/jqadm/post/media/decorators/global
		 ...
		 *
		 *  admin/jqadm/post/media/decorators/global = array( 'decorator1' )
		 *
		 ...
		 * @category Developer
		 * @see admin/jqadm/common/decorators/default
		 * @see admin/jqadm/post/media/decorators/excludes
		 * @see admin/jqadm/post/media/decorators/local
		 */

		/** admin/jqadm/post/media/decorators/local
		 ...
		 *
		 *  admin/jqadm/post/media/decorators/local = array( 'decorator2' )
		 *
		 ...
		 * @see admin/jqadm/post/media/decorators/global
		 */
		return $this->createSubClient( 'post/media/' . $type, $name );
	}
	 ...
	 *
	 * @param \Aimeos\MShop\Post\Item\Iface $item Post item including media reference
	 * @param array $listItems Media list items to be removed
	 * @return \Aimeos\MShop\Post\Item\Iface Modified post item
	 */
	protected function deleteMediaItems( \Aimeos\MShop\Post\Item\Iface $item, array $listItems ) : \Aimeos\MShop\Post\Item\Iface
	{
		$context = $this->getContext();
		...
		{
			$func = $search->make( 'post:has', ['media', $listItem->getType(), $listItem->getRefId()] );
	...
	{
		/** admin/jqadm/post/media/subparts
		 * List of JQAdm sub-clients rendered within the post media section
		 *
		 ...
		 * @category Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/post/media/subparts', [] );
	}
	...
	protected function fromArray( \Aimeos\MShop\Post\Item\Iface $item, array $data ) : \Aimeos\MShop\Post\Item\Iface
	{
		...
		foreach( $data as $idx => $entry )
		{
			if( ( $listItem = $item->getListItem( 'media', $entry['post.lists.type'], $entry['media.id'], false ) ) === null ) {
			...
			$refItem->fromArray( $entry, true )->setDomain( 'post' );
	...
	{
		/** admin/jqadm/post/media/template-item
		 * Relative path to the HTML body template of the media subpart for posts.
		 *
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/post/media/template-item';
		$default = 'post/item-media-standard';
