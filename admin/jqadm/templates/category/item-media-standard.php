...
/** admin/jqadm/category/item/media/config/suggest
 * List of suggested configuration keys in category media panel
 *
					...
					<div v-bind:id="'item-media-group-data-' + idx" v-bind:class="item['_show'] ? 'show' : 'collapsed'"
								...
								<div class="form-group row mandatory">
									...
									<div class="col-sm-8">
										<select is="select-component" required class="form-select listitem-type" tabindex="<?= $this->get( 'tabindex' ) ?>"
											...
											v-bind:name="`<?= $enc->js( $this->formparam( ['media', '_idx_', 'category.lists.type'] ) ) ?>`.replace('_idx_', idx)"
											v-bind:text="`<?= $enc->js( $this->translate( 'admin', 'Please select' ) ) ?>`"
							...
							<div class="form-group row optional">
								...
								<div class="col-sm-8">
									<input is="flat-pickr" class="form-control listitem-datestart" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ) ?>"
										v-bind:name="`<?= $enc->js( $this->formparam( ['media', '_idx_', 'category.lists.datestart'] ) ) ?>`.replace('_idx_', idx)"
							...
							<div class="form-group row optional">
								...
								<div class="col-sm-8">
									<input is="flat-pickr" class="form-control listitem-dateend" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ) ?>"
										v-bind:name="`<?= $enc->js( $this->formparam( ['media', '_idx_', 'category.lists.dateend'] ) ) ?>`.replace('_idx_', idx)"
										placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ) ?>"
										v-bind:disabled="item['category.lists.siteid'] != siteid"
										v-bind:config="Aimeos.flatpickr.datetime"
						...
						<div v-show="item['_ext']" class="col-xl-6 secondary" v-bind:class="{readonly: item['category.lists.siteid'] != siteid}">
							<config-table v-bind:tabindex="`<?= $enc->js( $this->get( 'tabindex' ) ) ?>`"
								v-bind:keys="<?= $enc->attr( $this->config( 'admin/jqadm/category/item/media/config/suggest', [] ) ) ?>"
