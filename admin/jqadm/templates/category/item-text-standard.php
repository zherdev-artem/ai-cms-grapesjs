...
/** admin/jqadm/category/item/text/config/suggest
 * List of suggested configuration keys in category text panel
 *
				...
				<div v-for="(item, idx) in items" v-bind:key="idx" class="group-item card">
					...
					<div v-bind:id="'item-text-group-item-' + idx" v-bind:class="item['_show'] ? 'show' : 'collapsed'"
						...
						<div class="card-tools-end">
							...
							<div v-if="item['category.lists.siteid'] == siteid && !item['_nosort']"
								class="btn btn-card-header act-move fa" tabindex="<?= $this->get( 'tabindex' ) ?>"
								title="<?= $enc->attr( $this->translate( 'admin', 'Move this entry up/down' ) ) ?>">
							</div>
							<div v-if="item['category.lists.siteid'] == siteid"
					...
					<div v-bind:id="'item-text-group-data-' + idx" v-bind:class="item['_show'] ? 'show' : 'collapsed'"
						...
						<div v-show="item['_ext']" class="col-xl-6 secondary">
								...
								<div class="form-group row mandatory">
									...
									<div class="col-sm-8">
										<select is="select-component" required class="form-select listitem-type" tabindex="<?= $enc->attr( $this->get( 'tabindex' ) ) ?>"
											v-bind:items="<?= $enc->attr( $listTypes->col( 'category.lists.type.label', 'category.lists.type.code' )->toArray() ) ?>"
											v-bind:name="`<?= $enc->js( $this->formparam( ['text', '_idx_', 'category.lists.type'] ) ) ?>`.replace('_idx_', idx)"
											v-bind:text="`<?= $enc->js( $this->translate( 'admin', 'Please select' ) ) ?>`"
											v-bind:readonly="item['category.lists.siteid'] != siteid"
											v-model="item['category.lists.type']" >
										</select>
									</div>
									<div class="col-sm-12 form-text text-muted help-text">
							...
							<?php else : ?>
								<input class="listitem-type" type="hidden"
									v-bind:name="`<?= $enc->js( $this->formparam( array( 'text', '_idx_', 'category.lists.type' ) ) ) ?>`.replace('_idx_', idx)"
							...
							<div class="form-group row optional">
								...
								<div class="col-sm-8">
									<input is="flat-pickr" class="form-control listitem-datestart" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ) ?>"
										v-bind:name="`<?= $enc->js( $this->formparam( array( 'text', '_idx_', 'category.lists.datestart' ) ) ) ?>`.replace('_idx_', idx)"
										placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ) ?>"
										v-bind:disabled="item['category.lists.siteid'] != siteid"
										v-bind:config="Aimeos.flatpickr.datetime"
										v-model="item['category.lists.datestart']" />
								</div>
								<div class="col-sm-12 form-text text-muted help-text">
							...
							<div class="form-group row optional">
								...
								<div class="col-sm-8">
									<input is="flat-pickr" class="form-control listitem-dateend" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ) ?>"
										v-bind:name="`<?= $enc->js( $this->formparam( array( 'text', '_idx_', 'category.lists.dateend' ) ) ) ?>`.replace('_idx_', idx)"
										placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ) ?>"
										v-bind:disabled="item['category.lists.siteid'] != siteid"
										v-bind:config="Aimeos.flatpickr.datetime"
										v-model="item['category.lists.dateend']" />
								</div>
								<div class="col-sm-12 form-text text-muted help-text">
						...
						<div v-show="item['_ext']" class="col-xl-6 secondary" v-bind:class="{readonly: item['category.lists.siteid'] != siteid}">
							<config-table v-bind:tabindex="`<?= $enc->js( $this->get( 'tabindex' ) ) ?>`"
								v-bind:keys="<?= $enc->attr( $this->config( 'admin/jqadm/category/item/text/config/suggest', [] ) ) ?>"
								v-bind:name="`<?= $enc->js( $this->formparam( ['text', '_idx_', 'config', '_pos_', '_key_'] ) ) ?>`.replace('_idx_', idx)"
								v-bind:index="idx" v-bind:readonly="item['category.lists.siteid'] != siteid"
