...
/** admin/jqadm/category/item/config/suggest
 * List of suggested configuration keys in category item panel
 *
 * Category items can store arbitrary key value pairs. This setting gives editors
 ...
 */
$cfgSuggest = $this->config( 'admin/jqadm/category/item/config/suggest', ['css-class'] );


?>
<?php $this->block()->start( 'jqadm_content' ) ?>

<form class="item item-category item-tree form-horizontal container-fluid" method="POST" enctype="multipart/form-data"
	action="<?= $enc->attr( $this->url( $target, $cntl, $action, $params, [], $config ) ) ?>"
	data-rootid="<?= $enc->attr( $this->get( 'itemRootId' ) ) ?>"
	data-geturl="<?= $enc->attr( $this->url( $getTarget, $getCntl, $getAction, ['resource' => 'category', 'id' => '_ID_'] + $params, [], $getConfig ) ) ?>"
	data-createurl="<?= $enc->attr( $this->url( $newTarget, $newCntl, $newAction, ['resource' => 'category', 'id' => '_ID_'] + $params, [], $newConfig ) ) ?>"
	data-jsonurl="<?= $enc->attr( $this->url( $jsonTarget, $jsonCntl, $jsonAction, ['resource' => 'category'] + $params, [], $jsonConfig ) ) ?>"
	data-idname="<?= $this->formparam( 'id' ) ?>" >

	<input id="item-id" type="hidden" name="<?= $enc->attr( $this->formparam( array( 'item', 'category.id' ) ) ) ?>"
		value="<?= $enc->attr( $this->get( 'itemData/category.id' ) ) ?>" />
	<input id="item-parentid" type="hidden" name="<?= $enc->attr( $this->formparam( array( 'item', 'category.parentid' ) ) ) ?>"
		value="<?= $enc->attr( $this->get( 'itemData/category.parentid', $this->param( 'parentid', $this->param( 'id', $this->get( 'itemRootId' ) ) ) ) ) ?>" />
	...
	<nav class="main-navbar">
		<h1 class="navbar-brand">
			...
			<span class="navbar-id"><?= $enc->html( $this->get( 'itemData/category.id' ) ) ?></span>
			<span class="navbar-label"><?= $enc->html( $this->get( 'itemData/category.label' ) ?: $this->translate( 'admin', 'New' ) ) ?></span>
			<span class="navbar-site"><?= $enc->html( $this->site()->match( $this->get( 'itemData/category.siteid' ) ) ) ?></span>
		</h1>
	...
	<div class="row item-container">
		...
		<div class="col-lg-3 category-tree">
			<div class="box">
				...
				<div class="row">
						...
						<div class="navbar-content">
							...
							<div class="item-meta text-muted">
								<small>
									...
									<span class="meta-value"><?= $enc->html( $this->get( 'itemData/category.mtime' ) ) ?></span>
								</small>
								<small>
									<?= $enc->html( $this->translate( 'admin', 'Created' ) ) ?>:
									<span class="meta-value"><?= $enc->html( $this->get( 'itemData/category.ctime' ) ) ?></span>
								</small>
								<small>
									<?= $enc->html( $this->translate( 'admin', 'Editor' ) ) ?>:
									<span class="meta-value"><?= $enc->html( $this->get( 'itemData/category.editor' ) ) ?></span>
								</small>
							</div>
					...
					<div class="col-xl-12 item-content tab-content">
						...
						<div id="basic" class="item-basic tab-pane fade show active" role="tabpanel" aria-labelledby="basic">
							...
							<div class="box">
								<div class="row">
									<div class="col-xl-6 <?= $this->site()->readonly( $this->get( 'itemData/category.siteid' ) ) ?>">
										<div class="form-group row mandatory">
											<label class="col-sm-4 form-control-label"><?= $enc->html( $this->translate( 'admin', 'Status' ) ) ?></label>
											<div class="col-sm-8">
												<select class="form-select item-status" required="required" tabindex="1"
													name="<?= $enc->attr( $this->formparam( array( 'item', 'category.status' ) ) ) ?>"
													<?= $this->site()->readonly( $this->get( 'itemData/category.siteid' ) ) ?> >
													<option value="">
														<?= $enc->html( $this->translate( 'admin', 'Please select' ) ) ?>
													</option>
													<option value="1" <?= $selected( $this->get( 'itemData/category.status', 1 ), 1 ) ?> >
														<?= $enc->html( $this->translate( 'mshop/code', 'status:1' ) ) ?>
													</option>
													<option value="0" <?= $selected( $this->get( 'itemData/category.status', 1 ), 0 ) ?> >
														<?= $enc->html( $this->translate( 'mshop/code', 'status:0' ) ) ?>
													</option>
													<option value="-1" <?= $selected( $this->get( 'itemData/category.status', 1 ), -1 ) ?> >
														<?= $enc->html( $this->translate( 'mshop/code', 'status:-1' ) ) ?>
													</option>
													<option value="-2" <?= $selected( $this->get( 'itemData/category.status', 1 ), -2 ) ?> >
										...
										<div class="form-group row mandatory">
											...
											<div class="col-sm-8">
												<input class="form-control item-code" type="text" required="required" tabindex="1"
													name="<?= $enc->attr( $this->formparam( array( 'item', 'category.code' ) ) ) ?>"
													placeholder="<?= $enc->attr( $this->translate( 'admin', 'Unique category code (required)' ) ) ?>"
													value="<?= $enc->attr( $this->get( 'itemData/category.code' ) ) ?>"
													<?= $this->site()->readonly( $this->get( 'itemData/category.siteid' ) ) ?> />
											</div>
										...
										<div class="form-group row mandatory">
											...
											<div class="col-sm-8">
												<input class="form-control item-label" type="text" required="required" tabindex="1"
													name="<?= $this->formparam( array( 'item', 'category.label' ) ) ?>"
													placeholder="<?= $enc->attr( $this->translate( 'admin', 'Internal name (required)' ) ) ?>"
													value="<?= $enc->attr( $this->get( 'itemData/category.label' ) ) ?>"
													<?= $this->site()->readonly( $this->get( 'itemData/category.siteid' ) ) ?> />
											</div>
											<div class="col-sm-12 form-text text-muted help-text">
										...
										<div class="form-group row optional advanced">
											...
											<div class="col-sm-8">
												<input class="form-control item-label" type="text" tabindex="1"
													name="<?= $this->formparam( array( 'item', 'category.url' ) ) ?>"
													placeholder="<?= $enc->attr( $this->translate( 'admin', 'Name in URL (optional)' ) ) ?>"
													value="<?= $enc->attr( $this->get( 'itemData/category.url' ) ) ?>"
													<?= $this->site()->readonly( $this->get( 'itemData/category.siteid' ) ) ?> />
											</div>
											<div class="col-sm-12 form-text text-muted help-text">
										...
										<div class="form-group row optional advanced warning">
											...
											<div class="col-sm-8">
												<input class="form-control item-target" type="text" tabindex="1"
													name="<?= $enc->attr( $this->formparam( array( 'item', 'category.target' ) ) ) ?>"
													placeholder="<?= $enc->attr( $this->translate( 'admin', 'Route or page ID (optional)' ) ) ?>"
													value="<?= $enc->attr( $this->get( 'itemData/category.target' ) ) ?>"
													<?= $this->site()->readonly( $this->get( 'itemData/category.siteid' ) ) ?> />
											</div>
											<div class="col-sm-12 form-text text-muted help-text">
									...
									--><div class="col-xl-6 vue <?= $this->site()->readonly( $this->get( 'itemData/category.siteid' ) ) ?>"
										data-data="<?= $enc->attr( $this->get( 'itemData', new stdClass() ) ) ?>">

										<config-table
											v-bind:keys="<?= $enc->attr( $this->config( 'admin/jqadm/category/item/config/suggest', ['css-class'] ) ) ?>"
											v-bind:name="`<?= $enc->js( $this->formparam( array( 'item', 'config', '_pos_', '_key_' ) ) ) ?>`"
											v-bind:readonly="data['category.siteid'] != `<?= $enc->js( $this->site()->siteid() ) ?>`"
