...
<form class="item item-post item-tree form-horizontal container-fluid" method="POST" enctype="multipart/form-data"
	action="<?= $enc->attr( $this->url( $target, $cntl, $action, $params, [], $config ) ) ?>"
	data-rootid="<?= $enc->attr( $this->get( 'itemRootId' ) ) ?>"
	data-geturl="<?= $enc->attr( $this->url( $getTarget, $getCntl, $getAction, ['resource' => 'post', 'id' => '_ID_'] + $params, [], $getConfig ) ) ?>"
	data-createurl="<?= $enc->attr( $this->url( $newTarget, $newCntl, $newAction, ['resource' => 'post', 'id' => '_ID_'] + $params, [], $newConfig ) ) ?>"
	data-jsonurl="<?= $enc->attr( $this->url( $jsonTarget, $jsonCntl, $jsonAction, ['resource' => 'post'], [], $jsonConfig ) ) ?>"
	data-idname="<?= $this->formparam( 'id' ) ?>" >

	<input id="item-id" type="hidden" name="<?= $enc->attr( $this->formparam( array( 'item', 'post.id' ) ) ) ?>"
		value="<?= $enc->attr( $this->get( 'itemData/post.id' ) ) ?>" />
	<input id="item-parentid" type="hidden" name="<?= $enc->attr( $this->formparam( array( 'item', 'post.parentid' ) ) ) ?>"
		value="<?= $enc->attr( $this->get( 'itemData/post.parentid', $this->param( 'parentid', $this->param( 'id', $this->get( 'itemRootId' ) ) ) ) ) ?>" />
	...
	<nav class="main-navbar">
		<h1 class="navbar-brand">
			...
			<span class="navbar-id"><?= $enc->html( $this->get( 'itemData/post.id' ) ) ?></span>
			<span class="navbar-label"><?= $enc->html( $this->get( 'itemData/post.label' ) ?: $this->translate( 'admin', 'New' ) ) ?></span>
			<span class="navbar-site"><?= $enc->html( $this->site()->match( $this->get( 'itemData/post.siteid' ) ) ) ?></span>
		</h1>
	...
	<div class="row item-container">
		...
		<?php if( isset( $this->itemData ) ) : ?>
			<div class="col-xl-12 post-content">
				<div class="row">
						...
						<div class="navbar-content">
							...
							<div class="item-meta text-muted">
								<small>
									...
									<span class="meta-value"><?= $enc->html( $this->get( 'itemData/post.mtime' ) ) ?></span>
								</small>
								<small>
									<?= $enc->html( $this->translate( 'admin', 'Created' ) ) ?>:
									<span class="meta-value"><?= $enc->html( $this->get( 'itemData/post.ctime' ) ) ?></span>
								</small>
								<small>
									<?= $enc->html( $this->translate( 'admin', 'Editor' ) ) ?>:
									<span class="meta-value"><?= $enc->html( $this->get( 'itemData/post.editor' ) ) ?></span>
								</small>
							</div>
					...
					<div class="col-xl-12 item-content tab-content">
						...
						<div id="basic" class="item-basic tab-pane fade show active" role="tabpanel" aria-labelledby="basic">
							...
							<div class="box">
								<div class="row">
									<div class="col-xl-6 <?= $this->site()->readonly( $this->get( 'itemData/post.siteid' ) ) ?>">
										<div class="form-group row mandatory">
											<label class="col-sm-4 form-control-label"><?= $enc->html( $this->translate( 'admin', 'Status' ) ) ?></label>
											<div class="col-sm-8">
												<select class="form-select item-status" required="required" tabindex="1"
													name="<?= $enc->attr( $this->formparam( array( 'item', 'post.status' ) ) ) ?>"
													<?= $this->site()->readonly( $this->get( 'itemData/post.siteid' ) ) ?> >
													<option value="">
														<?= $enc->html( $this->translate( 'admin', 'Please select' ) ) ?>
													</option>
													<option value="1" <?= $selected( $this->get( 'itemData/post.status', 1 ), 1 ) ?> >
														<?= $enc->html( $this->translate( 'mshop/code', 'status:1' ) ) ?>
													</option>
													<option value="0" <?= $selected( $this->get( 'itemData/post.status', 1 ), 0 ) ?> >
														<?= $enc->html( $this->translate( 'mshop/code', 'status:0' ) ) ?>
													</option>
													<option value="-1" <?= $selected( $this->get( 'itemData/post.status', 1 ), -1 ) ?> >
														<?= $enc->html( $this->translate( 'mshop/code', 'status:-1' ) ) ?>
													</option>
													<option value="-2" <?= $selected( $this->get( 'itemData/post.status', 1 ), -2 ) ?> >
										...
										<div class="form-group row mandatory">
											...
											<div class="col-sm-8">
												<input class="form-control item-label" type="text" required="required" tabindex="1"
													name="<?= $this->formparam( array( 'item', 'post.label' ) ) ?>"
													placeholder="<?= $enc->attr( $this->translate( 'admin', 'Internal name (required)' ) ) ?>"
													value="<?= $enc->attr( $this->get( 'itemData/post.label' ) ) ?>"
													<?= $this->site()->readonly( $this->get( 'itemData/post.siteid' ) ) ?> />
											</div>
											<div class="col-sm-12 form-text text-muted help-text">
