	...
	implements \Aimeos\Client\JsonApi\Iface
{
	...
	public function get( ServerRequestInterface $request, ResponseInterface $response ) : \Psr\Http\Message\ResponseInterface
	{
		...
		/** client/jsonapi/category/template
		 * Relative path to the category lists JSON API template
		 *
		 ...
		 * @category Developer
		 */
		$tplconf = 'client/jsonapi/category/template';
	...
	protected function getItem( \Aimeos\MW\View\Iface $view, ServerRequestInterface $request, ResponseInterface $response ) : \Psr\Http\Message\ResponseInterface
	{
		...
		if( in_array( 'category', $ref, true ) )
		{
			/** client/jsonapi/category/deep
			 * Load the category tree instead of the nodes of the first level only
			 *
			 * If you want to use the category filter component to display the whole
			 ...
			 * @see client/html/category/filter/tree/deep
			 */
			$deep = $view->config( 'client/jsonapi/category/deep', false );

			$level = $deep ? \Aimeos\MW\Tree\Manager\Base::LEVEL_TREE : \Aimeos\MW\Tree\Manager\Base::LEVEL_LIST;
		}

		$total = 1;
		$cntl = \Aimeos\Controller\Frontend::create( $this->getContext(), 'category' )->uses( $ref )
