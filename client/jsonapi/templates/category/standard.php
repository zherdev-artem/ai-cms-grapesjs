...
$entryFcn = function( \Aimeos\MShop\Category\Item\Iface $item ) use ( $fields, $target, $cntl, $action, $config )
{
...
$catFcn = function( \Aimeos\MShop\Category\Item\Iface $item, array $entry ) use ( $target, $cntl, $action, $config )
{
	$params = ['resource' => 'category', 'id' => $item->getId()];
	$entry['links'] = [
		'self' => [
			'href' => $this->url( $target, $cntl, $action, $params, [], $config ),
			'allow' => ['GET']
...
?>
{
	...
	<?php elseif( isset( $this->items ) ) : ?>
		<?php
			...
			$included = $this->jincluded( $items, $fields, ['category' => $catFcn] );
