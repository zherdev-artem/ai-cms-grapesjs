...
class Standard
	...
	implements \Aimeos\MShop\Category\Item\Iface
{
	 ...
	 * @param \Aimeos\MShop\Category\Item\Iface[] $children List of children of the item
		...
		array $listItems = [], array $refItems = [] )
	{
		...
		\Aimeos\MW\Common\Base::checkClassList( \Aimeos\MShop\Category\Item\Iface::class, $children );
	...
	/**
	 * Clones internal objects of the category item.
	 */
	...
	/**
	 * Returns the config property of the category.
	 *
	 * @return array Returns the config of the category node
	 */
	...
	/**
	 * Sets the config property of the category item.
	 *
	 * @param array $options Options to be set for the category node
	 * @return \Aimeos\MShop\Category\Item\Iface Category item for chaining method calls
	 */
	 ...
	 * @return \Aimeos\MShop\Category\Item\Iface Selected node
	 */
	public function getChild( int $index ) : \Aimeos\MShop\Common\Item\Tree\Iface
	{
		...
		throw new \Aimeos\MShop\Category\Exception( sprintf( 'Child node with index "%1$d" not available', $index ) );
	}
	 ...
	 *
	 * @return \Aimeos\Map Numerically indexed list of children implementing \Aimeos\MShop\Category\Item\Iface
	 */
	 ...
	 *
	 * @return \Aimeos\Map List of removed children implementing \Aimeos\MShop\Category\Item\Iface
	 */
	...
	public function fromArray( array &$list, bool $private = false ) : \Aimeos\MShop\Common\Item\Iface
	{
		...
		foreach( $list as $key => $value )
		{
			switch( $key )
			{
				case 'category.url': $item = $item->setUrl( $value ); break;
				case 'category.code': $item = $item->setCode( $value ); break;
				case 'category.label': $item = $item->setLabel( $value ); break;
				case 'category.target': $item = $item->setTarget( $value ); break;
				case 'category.status': $item = $item->setStatus( (int) $value ); break;
				case 'category.config': $item = $item->setConfig( (array) $value ); break;
				case 'category.id': !$private ?: $item = $item->setId( $value ); break;
				default: continue 2;
			}
	...
	{
		$list = [
			'category.url' => $this->getUrl(),
			'category.code' => $this->getCode(),
			'category.label' => $this->getLabel(),
			'category.config' => $this->getConfig(),
			'category.status' => $this->getStatus(),
			'category.target' => $this->getTarget(),
			'category.hasChildren' => $this->hasChildren(),
		];

		if( $private === true )
		{
			$list['category.id'] = $this->getId();
			$list['category.level'] = $this->getLevel();
			$list['category.siteid'] = $this->getSiteId();
			$list['category.parentid'] = $this->getParentId();
			$list['category.ctime'] = $this->getTimeCreated();
			$list['category.mtime'] = $this->getTimeModified();
			$list['category.editor'] = $this->getEditor();
		}
	 ...
	 *
	 * @return \Aimeos\Map List of IDs as keys and items implementing \Aimeos\MShop\Category\Item\Iface
	 */
