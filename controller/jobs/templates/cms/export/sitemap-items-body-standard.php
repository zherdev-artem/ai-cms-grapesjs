...
$detailTarget = $this->config( 'client/html/cms/url/target' );
$detailCntl = $this->config( 'client/html/cms/url/controller', 'page' );
$detailAction = $this->config( 'client/html/cms/url/action', 'index' );
$detailFilter = array_flip( $this->config( 'client/html/cms/url/filter', [] ) );
$detailConfig = $this->config( 'client/html/cms/url/config', [] );
...
foreach( $this->get( 'siteItems', [] ) as $id => $item )
{
	...
	{
		$params = array_diff_key( ['locale' => $lang, 'path' => \Aimeos\MW\Str::slug( $name )], $detailFilter );
