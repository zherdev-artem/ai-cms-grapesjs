...
namespace Aimeos\Controller\Jobs\Post\Export\Sitemap;


/**
 * Job controller for post sitemap.
 *
 * @package Controller
 * @subpackage Jobs
 */
	...
	implements \Aimeos\Controller\Jobs\Iface
{
	...
	{
		return $this->getContext()->getI18n()->dt( 'controller/jobs', 'Post site map' );
	}
	...
	{
		return $this->getContext()->getI18n()->dt( 'controller/jobs', 'Creates a post site map for search engines' );
	}
	...
	/**
	 * Adds the given posts to the content object for the site map file
	 *
	 * @param \Aimeos\MW\Container\Content\Iface $content File content object
	 * @param \Aimeos\Map $items List of post items implementing \Aimeos\MShop\Post\Item\Iface
	 */
	protected function addItems( \Aimeos\MW\Container\Content\Iface $content, \Aimeos\Map $items )
	{
		$config = $this->getContext()->getConfig();

		/** controller/jobs/post/export/sitemap/changefreq
		 * Change frequency of the posts
		 *
		 * Depending on how often the post content changes (e.g. price updates)
		 ...
		 * @category User
		 * @category Developer
		 * @see controller/jobs/post/export/sitemap/container/options
		 * @see controller/jobs/post/export/sitemap/location
		 * @see controller/jobs/post/export/sitemap/max-items
		 * @see controller/jobs/post/export/sitemap/max-query
		 */
		$changefreq = $config->get( 'controller/jobs/post/export/sitemap/changefreq', 'daily' );

		/** controller/jobs/post/export/sitemap/template-items
		 * Relative path to the XML items template of the post site map job controller.
		 *
		 ...
		 * @since 2015.01
		 * @category Developer
		 * @see controller/jobs/post/export/sitemap/template-header
		 * @see controller/jobs/post/export/sitemap/template-footer
		 * @see controller/jobs/post/export/sitemap/template-index
		 */
		$tplconf = 'controller/jobs/post/export/sitemap/template-items';
		$default = 'post/export/sitemap-items-body-standard';
	...
	protected function createContainer() : \Aimeos\MW\Container\Iface
	{
		...
		/** controller/jobs/post/export/sitemap/location
		 ...
		 * posts. Please make sure that the protocol and domain
		 ...
		 * @see controller/jobs/post/export/sitemap/max-items
		 * @see controller/jobs/post/export/sitemap/max-query
		 * @see controller/jobs/post/export/sitemap/changefreq
		 */
		$location = $config->get( 'controller/jobs/post/export/sitemap/location' );

		/** controller/jobs/post/export/sitemap/container/options
		...
		$options = $config->get( 'controller/jobs/post/export/sitemap/container/options', $default );

		if( $location == null )
		{
			$msg = sprintf( 'Required configuration for "%1$s" is missing', 'controller/jobs/post/export/sitemap/location' );
	...
	{
		/** controller/jobs/post/export/sitemap/template-header
		 * Relative path to the XML site map header template of the post site map job controller.
		 *
		 * The template file contains the XML code and processing instructions
		 ...
		 * @since 2015.01
		 * @category Developer
		 * @see controller/jobs/post/export/sitemap/template-items
		 * @see controller/jobs/post/export/sitemap/template-footer
		 * @see controller/jobs/post/export/sitemap/template-index
		 */
		$tplconf = 'controller/jobs/post/export/sitemap/template-header';
		$default = 'post/export/sitemap-items-header-standard';
	...
	{
		/** controller/jobs/post/export/sitemap/template-footer
		 * Relative path to the XML site map footer template of the post site map job controller.
		 *
		 * The template file contains the XML code and processing instructions
		 ...
		 * @see controller/jobs/post/export/sitemap/template-index
		 */
		$tplconf = 'controller/jobs/post/export/sitemap/template-footer';
		$default = 'post/export/sitemap-items-footer-standard';
	...
	protected function createSitemapIndex( \Aimeos\MW\Container\Iface $container, array $files )
	{
		...
		/** controller/jobs/post/export/sitemap/template-index
		 * Relative path to the XML site map index template of the post site map job controller.
		 *
		 * The template file contains the XML code and processing instructions
		 ...
		 * @see controller/jobs/post/export/sitemap/template-footer
		 */
		$tplconf = 'controller/jobs/post/export/sitemap/template-index';
		$default = 'post/export/sitemap-index-standard';

		/** controller/jobs/post/export/sitemap/baseurl
		...
		$baseUrl = $config->get( 'controller/jobs/post/export/sitemap/baseurl', $baseUrl );

		if( $baseUrl == null )
		{
			$msg = sprintf( 'Required configuration for "%1$s" is missing', 'controller/jobs/post/export/sitemap/baseurl' );
			throw new \Aimeos\Controller\Jobs\Exception( $msg );
		}
		...
		$content = $container->create( 'aimeos-post-sitemap-index.xml' );
	...
	/**
	 * Exports the posts into the given container
	 *
	 * @param \Aimeos\MW\Container\Iface $container Container object
	 * @param bool $default True to filter exported posts by default criteria
	...
	protected function getConfig( string $name, $default = null )
	{
		...
		{
			case 'domains':
				/** controller/jobs/post/export/sitemap/domains
				 * List of associated items from other domains that should be fetched for the sitemap
				 *
				 * Posts consist not only of the base data but also of texts, media
				 * files, prices, attrbutes and other details. Those information is
				 * associated to the posts via their lists. Using the "domains" option
				 ...
				 * @see controller/jobs/post/export/sitemap/changefreq
				 */
				return $config->get( 'controller/jobs/post/export/sitemap/domains', $default );

			case 'max-items':
				/** controller/jobs/post/export/sitemap/max-items
				 * Maximum number of posts per site map
				 *
				 * Each site map file must not contain more than 50,000 links and it's
				 * size must be less than 10MB. If your post URLs are rather long
				 * and one of your site map files is bigger than 10MB, you should set
				 * the number of posts per file to a smaller value until each file
				 * is less than 10MB.
				 *
				 * More details about site maps can be found at
				 * {@link http://www.sitemaps.org/protocol.html sitemaps.org}
				 *
				 * @param integer Number of posts per file
				 * @since 2015.01
				 * @category Developer
				 * @category User
				 * @see controller/jobs/post/export/sitemap/container/options
				 ...
				 * @see controller/jobs/post/export/sitemap/max-query
				 * @see controller/jobs/post/export/sitemap/changefreq
				 * @see controller/jobs/post/export/sitemap/domains
				 */
				return $config->get( 'controller/jobs/post/export/sitemap/max-items', 50000 );

			case 'max-query':
				/** controller/jobs/post/export/sitemap/max-query
				 * Maximum number of posts per query
				 *
				 * The posts are fetched from the database in bunches for efficient
				 ...
				 * {@see controller/jobs/post/export/sitemap/max-items max-items}
				 *
				 * @param integer Number of posts per query
				 * @since 2015.01
				 * @category Developer
				 * @see controller/jobs/post/export/sitemap/container/options
				 * @see controller/jobs/post/export/sitemap/location
				 ...
				 * @see controller/jobs/post/export/sitemap/domains
				 */
				return $config->get( 'controller/jobs/post/export/sitemap/max-query', 1000 );
		}
	...
	{
		return sprintf( 'aimeos-post-sitemap-%d.xml', $number );
	}
}
