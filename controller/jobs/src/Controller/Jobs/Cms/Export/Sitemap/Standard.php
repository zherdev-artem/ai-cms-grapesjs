...
namespace Aimeos\Controller\Jobs\Cms\Export\Sitemap;


/**
 * Job controller for cms sitemap.
 *
 * @package Controller
 * @subpackage Jobs
 */
	...
	implements \Aimeos\Controller\Jobs\Iface
{
	...
	{
		return $this->getContext()->getI18n()->dt( 'controller/jobs', 'Cms site map' );
	}
	...
	{
		return $this->getContext()->getI18n()->dt( 'controller/jobs', 'Creates a cms site map for search engines' );
	}
	...
	/**
	 * Adds the given cmss to the content object for the site map file
	 *
	 * @param \Aimeos\MW\Container\Content\Iface $content File content object
	 * @param \Aimeos\Map $items List of cms items implementing \Aimeos\MShop\Cms\Item\Iface
	 */
	protected function addItems( \Aimeos\MW\Container\Content\Iface $content, \Aimeos\Map $items )
	{
		$config = $this->getContext()->getConfig();

		/** controller/jobs/cms/export/sitemap/changefreq
		 * Change frequency of the cmss
		 *
		 * Depending on how often the cms content changes (e.g. price updates)
		 ...
		 * @category User
		 * @category Developer
		 * @see controller/jobs/cms/export/sitemap/container/options
		 * @see controller/jobs/cms/export/sitemap/location
		 * @see controller/jobs/cms/export/sitemap/max-items
		 * @see controller/jobs/cms/export/sitemap/max-query
		 */
		$changefreq = $config->get( 'controller/jobs/cms/export/sitemap/changefreq', 'daily' );

		/** controller/jobs/cms/export/sitemap/template-items
		 * Relative path to the XML items template of the cms site map job controller.
		 *
		 ...
		 * @since 2015.01
		 * @category Developer
		 * @see controller/jobs/cms/export/sitemap/template-header
		 * @see controller/jobs/cms/export/sitemap/template-footer
		 * @see controller/jobs/cms/export/sitemap/template-index
		 */
		$tplconf = 'controller/jobs/cms/export/sitemap/template-items';
		$default = 'cms/export/sitemap-items-body-standard';
	...
	protected function createContainer() : \Aimeos\MW\Container\Iface
	{
		...
		/** controller/jobs/cms/export/sitemap/location
		 ...
		 * cmss. Please make sure that the protocol and domain
		 ...
		 * @see controller/jobs/cms/export/sitemap/max-items
		 * @see controller/jobs/cms/export/sitemap/max-query
		 * @see controller/jobs/cms/export/sitemap/changefreq
		 */
		$location = $config->get( 'controller/jobs/cms/export/sitemap/location' );

		/** controller/jobs/cms/export/sitemap/container/options
		...
		$options = $config->get( 'controller/jobs/cms/export/sitemap/container/options', $default );

		if( $location == null )
		{
			$msg = sprintf( 'Required configuration for "%1$s" is missing', 'controller/jobs/cms/export/sitemap/location' );
	...
	{
		/** controller/jobs/cms/export/sitemap/template-header
		 * Relative path to the XML site map header template of the cms site map job controller.
		 *
		 * The template file contains the XML code and processing instructions
		 ...
		 * @since 2015.01
		 * @category Developer
		 * @see controller/jobs/cms/export/sitemap/template-items
		 * @see controller/jobs/cms/export/sitemap/template-footer
		 * @see controller/jobs/cms/export/sitemap/template-index
		 */
		$tplconf = 'controller/jobs/cms/export/sitemap/template-header';
		$default = 'cms/export/sitemap-items-header-standard';
	...
	{
		/** controller/jobs/cms/export/sitemap/template-footer
		 * Relative path to the XML site map footer template of the cms site map job controller.
		 *
		 * The template file contains the XML code and processing instructions
		 ...
		 * @see controller/jobs/cms/export/sitemap/template-index
		 */
		$tplconf = 'controller/jobs/cms/export/sitemap/template-footer';
		$default = 'cms/export/sitemap-items-footer-standard';
	...
	protected function createSitemapIndex( \Aimeos\MW\Container\Iface $container, array $files )
	{
		...
		/** controller/jobs/cms/export/sitemap/template-index
		 * Relative path to the XML site map index template of the cms site map job controller.
		 *
		 * The template file contains the XML code and processing instructions
		 ...
		 * @see controller/jobs/cms/export/sitemap/template-footer
		 */
		$tplconf = 'controller/jobs/cms/export/sitemap/template-index';
		$default = 'cms/export/sitemap-index-standard';

		/** controller/jobs/cms/export/sitemap/baseurl
		...
		$baseUrl = $config->get( 'controller/jobs/cms/export/sitemap/baseurl', $baseUrl );

		if( $baseUrl == null )
		{
			$msg = sprintf( 'Required configuration for "%1$s" is missing', 'controller/jobs/cms/export/sitemap/baseurl' );
			throw new \Aimeos\Controller\Jobs\Exception( $msg );
		}
		...
		$content = $container->create( 'aimeos-cms-sitemap-index.xml' );
	...
	/**
	 * Exports the cmss into the given container
	 *
	 * @param \Aimeos\MW\Container\Iface $container Container object
	 * @param bool $default True to filter exported cmss by default criteria
	...
	protected function export( \Aimeos\MW\Container\Iface $container, bool $default = true ) : array
	{
		...
		// ] ) );
		$search->setSortations( array( $search->sort( '+', 'cms.id' ) ) );
	...
	protected function getConfig( string $name, $default = null )
	{
		...
		{
			case 'domains':
				/** controller/jobs/cms/export/sitemap/domains
				 * List of associated items from other domains that should be fetched for the sitemap
				 *
				 * Cmss consist not only of the base data but also of texts, media
				 * files, prices, attrbutes and other details. Those information is
				 * associated to the cmss via their lists. Using the "domains" option
				 ...
				 * @see controller/jobs/cms/export/sitemap/changefreq
				 */
				return $config->get( 'controller/jobs/cms/export/sitemap/domains', $default );

			case 'max-items':
				/** controller/jobs/cms/export/sitemap/max-items
				 * Maximum number of cmss per site map
				 *
				 * Each site map file must not contain more than 50,000 links and it's
				 * size must be less than 10MB. If your cms URLs are rather long
				 * and one of your site map files is bigger than 10MB, you should set
				 * the number of cmss per file to a smaller value until each file
				 * is less than 10MB.
				 *
				 * More details about site maps can be found at
				 * {@link http://www.sitemaps.org/protocol.html sitemaps.org}
				 *
				 * @param integer Number of cmss per file
				 * @since 2015.01
				 * @category Developer
				 * @category User
				 * @see controller/jobs/cms/export/sitemap/container/options
				 ...
				 * @see controller/jobs/cms/export/sitemap/max-query
				 * @see controller/jobs/cms/export/sitemap/changefreq
				 * @see controller/jobs/cms/export/sitemap/domains
				 */
				return $config->get( 'controller/jobs/cms/export/sitemap/max-items', 50000 );

			case 'max-query':
				/** controller/jobs/cms/export/sitemap/max-query
				 * Maximum number of cmss per query
				 *
				 * The cmss are fetched from the database in bunches for efficient
				 ...
				 * {@see controller/jobs/cms/export/sitemap/max-items max-items}
				 *
				 * @param integer Number of cmss per query
				 * @since 2015.01
				 * @category Developer
				 * @see controller/jobs/cms/export/sitemap/container/options
				 * @see controller/jobs/cms/export/sitemap/location
				 ...
				 * @see controller/jobs/cms/export/sitemap/domains
				 */
				return $config->get( 'controller/jobs/cms/export/sitemap/max-query', 1000 );
		}
	...
	{
		return sprintf( 'aimeos-cms-sitemap-%d.xml', $number );
	}
}
